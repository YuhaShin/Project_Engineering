...
festival = df_temp.withColumn('tmp', explode(arrays_zip(col('temp.place'), col('temp.coord')))).select(col('tmp.place'), col('tmp.coord'), col('temp.period'), col('temp.time')).withColumn('from_dt', to_date(substring(split('period', '~').getItem(0), 1, 10), 'yyyy-MM-dd')).withColumn('to_dt', to_date(substring(split('period', '~').getItem(1), 2, 10), 'yyyy-MM-dd')).drop('period')
